...
// ## ИНФОРМАЦИЯ  
//   Для того чтобы прочитать файл Вам понадобится модуль fs из встроенной  
//   библиотеки Node.Для того чтобы загрузить этот, или любой другой, модуль  
//   из этой библиотеки воспользуйтесь следующей конструкцией:

//   const fs = require('fs')  

//   Таким образом fs модуль будет доступен в переменной fs.  

//   Все синхронные(или блокирующие) методы файловой системы в fs оканчиваются  
//   на Sync.Для того чтобы прочитать файл Вам понадобится
//   fs.readFileSync('/path/to/file').Этот метод вернет объект типа Buffer  
//   содержащий весь контент файла.  
...
//   над ними, например const str = buf.toString().  

//   Документацию для Buffer вы сможете найти набрав в браузере:
//   file:///Users/softmaker/.nvm/versions/node/v18.12.1/lib/node_modules/learnyounode/docs-nodejs/buffer.html  
